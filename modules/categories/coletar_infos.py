...
def banner_coletar_info(): ## banner da category coleta de informações
    banner()
    coleinfoxx = open("modules/banners/categ_coletar.txt")
...
def sherlock():
	if os.path.exists(f"{CasaDir}/sherlock") == True:
	    Installed_banner()
	    print(f"\n{vlh}Essa ferramenta já ser encontra no diretório ->{am}'/home'{vd}");timeout(1);
	elif os.path.exists(f"{Dirbin}/sherlock/") == True:
	    Installed_banner()
	    print(f"\n{vlh}Essa ferramenta já ser encontra no diretório ->{am}'/bin'{vd}");timeout(1);
	else:
	    install_banner()
	    ...
	    os.system("mv sherlock {}".format(CasaDir))
	    print(f"{v}\nINSTALAÇÃO CONCLUIDA ;) {b}")
	    backmenu()
