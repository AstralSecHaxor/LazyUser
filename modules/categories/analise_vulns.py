...
def banner_analise_vulne():
    banner()
    banner_analise = open("modules/banners/categ_analise.txt")
    print(banner_analise.read())
    backmenu_inicial()
...
def tmscanner():
	if os.path.exists(f"{CasaDir}/TM-scanner") == True:
	    Installed_banner()
	    print(f"\n{vlh}Essa ferramenta já ser encontra no diretório ->{am}'/home'{vd}");timeout(1);
	elif os.path.exists(f"{Dirbin}/TM-scanner") == True:
	    Installed_banner()
	    print(f"\n{vlh}Essa ferramenta já ser encontra no diretório ->{am}'/bin'{vd}");timeout(1);
	else:
	    install_banner()
